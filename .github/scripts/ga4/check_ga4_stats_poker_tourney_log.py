#!/usr/bin/env python3
"""
GA4(Google Analytics Data API)から、ポーカートナメ成績表(poker-tourney-log、
測定ID G-R7ZQLCWL2J)のPV・アクティブユーザー数等を取得する。

check_ga4_stats_poker_hand_media.py と同じ構成・認証(ga4_auth.py)を使う。
プロパティキャッシュのみ別ファイル(ga4_property_cache_poker_tourney_log.json)に
...
TARGET_MEASUREMENT_ID = "G-R7ZQLCWL2J"
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
PROPERTY_CACHE_PATH = os.path.join(SCRIPT_DIR, "ga4_property_cache_poker_tourney_log.json")

# GA4計測タグ導入日(2026-09-17、Web版公開自体は2026-08-28)。それ以前のデータは
# 計測していないため、sinceLaunchはタグ導入日からの集計とする。
SINCE_LAUNCH_DATE = "2026-09-17"
...
def run_report(data_service, property_path, start_date, end_date):
    ...
    # 抜けるためここに乗らず、PV/セッションより実際の読者数に近い指標になる。
    body = {
